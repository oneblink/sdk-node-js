...
const BaseSchema = Joi.object().keys({
  ...
  dataLookupUrl: Joi.when('type', {
    ...
    then: Joi.when('isDataLookup', {
      ...
      then: Joi.string()
        .uri()
        .required()
        .label('Data Lookup Url'),
      otherwise: Joi.any().strip()
    }),
    otherwise: Joi.any().strip()
  }),
  dataLookupApiId: Joi.when('type', {
    is: Joi.only(userInputTypes),
    then: Joi.when('isDataLookup', {
      is: true,
      then: Joi.string()
        .allow(null)
        .label('Data Lookup Api Id'),
      otherwise: Joi.any().strip()
    }),
    otherwise: Joi.any().strip()
  }),
  ...
  elementLookupUrl: Joi.when('type', {
    is: Joi.only(userInputTypes),
    then: Joi.when('isElementLookup', {
      ...
      then: Joi.string()
        .uri()
        .required()
        .label('Element Lookup Url'),
      otherwise: Joi.any().strip()
    }),
    otherwise: Joi.any().strip()
  }),
  elementLookupApiId: Joi.when('type', {
    is: Joi.only(userInputTypes),
    then: Joi.when('isElementLookup', {
      is: true,
      then: Joi.string()
        .allow(null)
        .label('Element Lookup Api Id'),
      otherwise: Joi.any().strip()
    }),
    otherwise: Joi.any().strip()
  }),
  ...
  // restricted file types for file element
...
const SubmissionEventsSchema = Joi.object().keys({
  ...
  type: Joi.string()
    ...
    .valid(['CALLBACK', 'PDF', 'SPOTTO', 'ONEBLINK_API']),
  configuration: Joi.allow(null)
    ...
    .when('type', {
      ...
      then: Joi.object().keys({
        apiId: Joi.string()
          .required()
          .regex(/^[a-z\d-]+\.api\.(?:oneblink|blinkm)\.io$/)
...
module.exports = {
  formSchema: Joi.object().keys({
    id: Joi.number(),
    environmentId: Joi.number()
      .label('Environment OneBlink')
      .required(),
    name: Joi.string()
      .label('Name')
      .required(),
