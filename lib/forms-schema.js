...
const BaseSchema = Joi.object().keys({
  ...
  'name': Joi.when('type', {
    ...
    then: Joi.any().strip(),
    otherwise: Joi.string().label('Form Element - Name').required()
  }),
  'label': Joi.when('type', {
    is: Joi.only(['form', 'infoPage']),
    then: Joi.any().strip(),
  ...
  'requiresAllConditionallyShowPredicates': Joi.when('conditionallyShow', {
    is: true,
    then: Joi.bool().default(false).label('Form Element - Requires All Conditionally Show Predicates are Met'),
    otherwise: Joi.any().strip()
  }),
  ...
  'buttons': Joi.when('type', {
    ...
    then: Joi.boolean().label('Form Element - Radio Buttons as Buttons').default(false),
    otherwise: Joi.any().strip()
  }),
  // Select
  'multi': Joi.when('type', {
  ...
  'isSlider': Joi.when('type', {
    is: 'number',
    then: Joi.boolean().label('Form Element - Display Number as Slider').default(false),
    otherwise: Joi.any().strip()
  }),
  'sliderIncrement': Joi.when('isSlider', {
    is: true,
    then: JoiRange.range().within({ min: Joi.ref('minNumber'), max: Joi.ref('maxNumber') }).label('Form Element - Slider Increment'),
    else: Joi.any().strip()
  }),
  ...
  'headingType': Joi.when('type', {
    is: Joi.only('heading'),
    then: Joi.number().required().label('Form Element - Heading Size').valid([
      1,
      2,
      3,
      4,
  ...
  'toDate': Joi.when('type', {
    ...
    then: Joi.when('fromDate', {
      ...
      then: Joi.date().iso().min(Joi.ref('fromDate')).label('Form Element - To Date')
    }).allow(null),
    otherwise: Joi.any().strip()
  }),
  ...
  'removeSetEntryLabel': Joi
    .when('type', {
      is: Joi.only('repeatableSet'),
      then: Joi.string().label('Form Element - Remove repeatable set entry label'),
      otherwise: Joi.any().strip()
    }),
  'elements': Joi
    .when('type', {
      is: Joi.only('repeatableSet'),
      then: Joi.array().items(Joi.lazy(() => BaseSchema)).required().min(1).unique('name', { ignoreUndefined: true }).unique('id').label('Form Element - Repeatable Set - Elements'),
      otherwise: Joi.any().strip()
    }),
  ...
  'restrictBarcodeTypes': Joi
    .when('type', {
      ...
      then: Joi.boolean().default(false).label('Form Element - Barcode Scanner - restrictBarcodeTypes'),
      otherwise: Joi.any().strip()
    }),
  'restrictedBarcodeTypes': Joi
    .when('type', {
      ...
      then: Joi.when('restrictBarcodeTypes', {
        is: true,
        then: Joi.array().items(Joi.string()).label('Form Element - Barcode Scanner - restrictedBarcodeTypes').required(),
        otherwise: Joi.any().strip()
      }),
      otherwise: Joi.any().strip()
    }),
  ...
  'calculation': Joi
    .when('type', {
      ...
      then: Joi.string().label('Form Element - Calculation - calculation').required(),
      otherwise: Joi.any().strip()
    }),
  ...
  'dataLookupUrl': Joi
    .when('type', {
      ...
      then: Joi.when('isDataLookup', {
        is: true,
        then: Joi.string().uri().required().label('Data Lookup Url'),
        otherwise: Joi.any().strip()
      }),
      otherwise: Joi.any().strip()
    }),
  'dataLookupApiId': Joi
    .when('type', {
      ...
      then: Joi.when('isDataLookup', {
        is: true,
        then: Joi.string().allow(null).label('Data Lookup Api Id'),
        otherwise: Joi.any().strip()
      }),
      otherwise: Joi.any().strip()
    }),
  ...
  'elementLookupUrl': Joi
    .when('type', {
      ...
      then: Joi.when('isElementLookup', {
        is: true,
        then: Joi.string().uri().required().label('Element Lookup Url'),
        otherwise: Joi.any().strip()
      }),
      otherwise: Joi.any().strip()
    }),
  'elementLookupApiId': Joi
    .when('type', {
      ...
      then: Joi.when('isElementLookup', {
        is: true,
        then: Joi.string().allow(null).label('Element Lookup Api Id'),
        otherwise: Joi.any().strip()
      }),
      otherwise: Joi.any().strip()
    }),
