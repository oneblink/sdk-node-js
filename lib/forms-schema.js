...
const BaseSchema = Joi.object().keys({
  ...
  // Date and Date+Time
  'fromDate': Joi.date().iso().label('Form Element - From Date')
    .when('type', {
  ...
  'addSetEntryLabel': Joi.string()
    .label('Form Element - Add repeatable set entry label')
    .when('type', {
      is: Joi.not('repeatableSet'),
      then: Joi.any().strip()
    }),
  'removeSetEntryLabel': Joi.string()
    .label('Form Element - Remove repeatable set entry label')
    .when('type', {
      is: Joi.not('repeatableSet'),
      then: Joi.any().strip()
    }),
  ...
  'restrictedBarcodeTypes': Joi.array()
    .label('Form Element - Barcode Scanner - restrictedBarcodeTypes')
    .when('type', {
      is: Joi.not('barcodeScanner'),
      then: Joi.any().strip()
    })
    .when('restrictBarcodeTypes', {
      is: true,
  ...
  'dataLookupApiId': Joi
    .when('type', {
      ...
      then: Joi.when('isDataLookup', {
        is: true,
        then: Joi.string().label('Data Lookup Api Id'),
        otherwise: Joi.allow(null)
      }),
      otherwise: Joi.any().strip()
    }),
  ...
  'isElementLookup': Joi.label('Element Lookup enabled')
    .when('type', {
      is: Joi.only(userInputTypes),
      then: Joi.boolean().default(false),
      otherwise: Joi.any().strip()
  ...
  'elementLookupApiId': Joi
    .when('type', {
      is: Joi.only(userInputTypes),
      then: Joi.string().label('Element Lookup Api Id'),
      otherwise: Joi.any().strip()
    })
    .when('isElementLookup', {
      is: true,
      then: Joi.string(),
      otherwise: Joi.allow(null)
    }),
