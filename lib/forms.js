      ...
      .then((results) => {
        ...
        if (prefillData) {
          return super.getRequest(`/forms/${formId}/pre-fill-credentials`)
            .then((prefillMeta) => {
              return prefillData(prefillMeta, prefillData)
                .then(() => prefillMeta.preFillFormDataId)
            })
        }
      })
      .then((preFillFormDataId /* : ?string */) => {
        // Default expiry for token is 8 hours
        const jwtExpiry = 28800

        const token = generateJWT(this.accessKey, this.secretKey, jwtExpiry)

        const formUrl = generateFormUrl(`https://${results.organisations[0].formsHostname}/forms`, formId, token, externalId, preFillFormDataId)
