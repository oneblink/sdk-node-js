...
| `newJob.details.priority`    | No       | `number` | Value used to order jobs by priority in the OneBlink system, with 1 being the higest priority. The OneBlink System will order jobs by priority and the date the job is created (oldest to newest). Jobs without a priority will appear below jobs with priority |
