...
test('should error if page element has child page element', () => {
  ...
  expect(error?.details[0].message).toContain(
    '"elements[0].elements[0].type" must be one of [abn, autocomplete, barcodeScanner, boolean, bsb, calculation, camera, captcha, checkboxes, civicaNameRecord, civicaStreetName, compliance, date, datetime, draw, email, files, form, freskdeskDependentField, geoscapeAddress, heading, html, image, infoPage, location, number, pointAddress, radio, repeatableSet, section, select, summary, telephone, text, textarea, time]',
  )
})
...
test('should error if isMultiPage is set to false', () => {
  ...
  expect(error?.details[0].message).toContain(
    '"elements[0].type" must be one of [abn, autocomplete, barcodeScanner, boolean, bsb, calculation, camera, captcha, checkboxes, civicaNameRecord, civicaStreetName, compliance, date, datetime, draw, email, files, form, freskdeskDependentField, geoscapeAddress, heading, html, image, infoPage, location, number, pointAddress, radio, repeatableSet, section, select, summary, telephone, text, textarea, time]',
  )
})
