...
describe('Scheduling events should throw errors when not passed to "schedulingEvents" property', () => {
  ...
  test('SCHEDULING form event should error when being passed to "submissionEvents"', () => {
    expect(() =>
      ...
      '"submissionEvents[0].type" must be one of [CALLBACK, POWER_AUTOMATE_FLOW, CP_INTEGRATION_HUB_WEBHOOK, PDF, EMAIL, ONEBLINK_API, TRIM, CP_HCMS, CIVICA_CRM, FRESHDESK_CREATE_TICKET, FRESHDESK_ADD_NOTE_TO_TICKET]',
    )
  })
...
describe('Payment events should throw errors when not passed to "paymentEvents" property', () => {
  ...
  test('Payment form event should error when being passed to "schedulingEvents"', () => {
    ...
    ).toThrow('"schedulingEvents[0].type" must be one of [NYLAS]')
  })
})
...
describe('"draftEvents" and "approvalEvents" should allow only submission events', () => {
  ...
  test('Payment form event should error when being passed to "draftEvents"', () => {
    expect(() =>
      ...
      '"draftEvents[0].type" must be one of [CALLBACK, POWER_AUTOMATE_FLOW, CP_INTEGRATION_HUB_WEBHOOK, PDF, EMAIL, ONEBLINK_API, TRIM, CP_HCMS, CIVICA_CRM, FRESHDESK_CREATE_TICKET, FRESHDESK_ADD_NOTE_TO_TICKET]',
    )
  })
  ...
  test('Payment form event should error when being passed to "approvalEvents"', () => {
    expect(() =>
      ...
      '"approvalEvents[0].type" must be one of [CALLBACK, POWER_AUTOMATE_FLOW, CP_INTEGRATION_HUB_WEBHOOK, PDF, EMAIL, ONEBLINK_API, TRIM, CP_HCMS, CIVICA_CRM, FRESHDESK_CREATE_TICKET, FRESHDESK_ADD_NOTE_TO_TICKET]',
    )
  })
