...
test('should error if "buttons" is not a boolean', () => {
  ...
  expect(error.message).toContain('"Form Element - Radio Buttons as Buttons" must be a boolean')
})
...
test('should error if element "id" is not unique', () => {
  ...
  expect(error.message).toContain('"Form Elements" position 1 contains a duplicate value')
})
...
test('should error if number min is greater than max', () => {
  ...
  expect(error.message).toContain('"Form Element - Maximum Number" must be larger than or equal to 33.5')
})
...
test('should error if "toDate" is greater than "fromDate"', () => {
  ...
  expect(error.message).toContain('"Form Element - To Date" must be larger than or equal to')
})
...
test('should error if "defaultValue" does not match what is valid for each type', () => {
  ...
  expect(error.details[0].message).toBe('"Form Element - Default Value" must be a string')
  expect(error.details[1].message).toBe('"Form Element - Default Value" must be a string')
  expect(error.details[2].message).toBe('"Form Element - Default Value" must be a number')
  expect(error.details[3].message).toBe('"Form Element - Default Date Value" must be a valid ISO 8601 date')
  expect(error.details[3].message).toBe('"Form Element - Default Date Value" must be a valid ISO 8601 date')
  expect(error.details[4].message).toBe('"Form Element - Default Date Value" must be a valid ISO 8601 date')
  expect(error.details[5].message).toBe('"Form Element - Default Date Value" must be a valid ISO 8601 date')
  expect(error.details[6].message).toBe('"Form Element - Default Value" must be a valid GUID')
  expect(error.details[7].message).toBe('"Form Element - Default Value" must be an array')
  expect(error.details[8].message).toBe('"0" must be a valid GUID')
  expect(error.details[9].message).toBe('"Form Element - Default Value" must be a valid GUID')
  expect(error.details[10].message).toBe('"Form Element - Default Value" must be an array')
  expect(error.details[11].message).toBe('"0" must be a valid GUID')
  expect(error.details[12].message).toBe('"Form Element - Default Value" must be a string')
  expect(error.details[13].message).toBe('"Form Element - Default Value" must be a string')
})
...
test('should error if number type element has a "defaultValue" less than "minNumber"', () => {
  ...
  expect(error.details[0].message).toBe('"Form Element - Default Value" must be larger than or equal to 5')
})
...
test('should error if number type element has a "defaultValue" more than "maxNumber"', () => {
  ...
  expect(error.details[0].message).toBe('"Form Element - Default Value" must be less than or equal to 2')
})
...
test('should error if date type element has a "defaultValue" less than "fromDate"', () => {
  ...
  expect(error.details[0].message).toContain('"Form Element - Default Date Value" must be larger than or equal to')
})
...
test('should error if date type element has a "defaultValue" more than "toDate"', () => {
  ...
  expect(error.details[0].message).toContain('"Form Element - Default Date Value" must be less than or equal to')
})
...
test('should error if repeatableSet type element has no elements', () => {
  ...
  expect(error.details[0].message).toContain('"Form Element - Repeatable Set - Elements" is required')
})
...
test('should error if repeatableSet type element min entries is more than max entries', () => {
  ...
  expect(error.details[0].message).toContain('"Form Element - Maximum number of repeatable set entries" must be larger than or equal to 2')
})
...
test('should error if repeatableSet type element min or max entries is less then 0', () => {
  ...
  expect(error.details[0].message).toContain('"Form Element - Minimum number of repeatable set entries" must be larger than or equal to 0')
  expect(error.details[1].message).toContain('"Form Element - Maximum number of repeatable set entries" must be larger than or equal to 0')
})
...
test('should error if page element has no elements', () => {
  ...
  expect(error.details[0].message).toContain('"Form Element - Page - Elements" must contain at least 1 items')
})
...
test('should error if image element does not have a default value', () => {
  ...
  expect(error.details[0].message).toBe('"Form Element - Default Value" is required')
})
...
test('should error if HTML element does not have a default value', () => {
  ...
  expect(error.details[1].message).toBe('"Form Element - Default Value" is required')
})
...
test('should error if calculation element does not have a default value', () => {
  ...
  expect(error.details[0].message).toBe('"Form Element - Default Value" is required')
  expect(error.details[1].message).toBe('"Form Element - Calculation - calculation" is required')
})
...
test('should throw error if postSubmissionAction is an invalid type', () => {
  ...
  expect(error.details[0].message).toBe('"Post Submission Action" must be one of [URL, CLOSE, FORMS_LIBRARY]')
})
...
test('should throw error if postSubmissionAction is URL but no URL is present', () => {
  ...
  expect(error.details[0].message).toBe('"Post Submission Redirect URL" is required')
})
...
test('should throw error if defaultValue for email is not valid', () => {
  ...
  expect(error.details[0].message).toBe('"Form Element - Default Email Value" must be a valid email')
})

test('should throw error if defaultValue contains src="data:', () => {
  const defaultValue = '<p>a<img src="https://git.blinkm.co/uploads/-/system/group/avatar/64/1blinkblack.png"><img src="data:image/jpeg;base64,/9j/4AA9k=" alt="Image result"></p>'
  const { error } = Joi.validate({
    'id': 1,
    'name': 'string',
    'description': 'string',
  ...
  expect(error.details[0].message).toBe(`"Form Element - Default Value" with value "${defaultValue}" matches the inverted No Binary Data pattern`)
})
...
describe('optionTypes', () => {
  describe('when dynamic', () => {
    ...
    test('invalid when dynamicOptionSetId is NOT provided', () => {
      ...
      expect(error.message).toContain('"Form Element - Dynamic Option Set Id" is required')
    })
  })
...
describe('PDF submission event', () => {
  ...
  test('should disallow PDF submission event with email that is not a email address or a field:', () => {
    ...
    expect(error.details[0].message).toBe(`"Form Submission Event - Email Address" must be a valid email`)
  })
})
...
describe('CALLBACK submission event', () => {
  ...
  test('should disallow CALLBACK submission event without url and secrent', () => {
    ...
    expect(error.details[0].message).toBe(`"Form Submission Event - Callback Url" is required`)
  })
})
...
describe('Conditional Predicates', () => {
  ...
  test('should error if conditional predicate types are missing required values', () => {
    ...
    expect(error.details[0].message).toBe('"Form Element - Conditionally Show Predicate - Option Ids" is required')
    expect(error.details[1].message).toBe('"Form Element - Conditionally Show Predicate - Operator" is required')
    expect(error.details[2].message).toBe('"Form Element - Conditionally Show Predicate - Value" is required')
  })
})
